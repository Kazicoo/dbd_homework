...
public class GamePanel extends JPanel {
    ...
    //         JButton button = clientGame.generators[i].getButton();  // 獲取每個發電機的按鈕
    
    //             if (button != null) {
    //                 int x = clientGame.generators[i].getX() - cameraOffsetX;  // 計算相對位置
    //                 int y = clientGame.generators[i].getY() - cameraOffsetY;
        
    //                 // 設置按鈕的位置
    //                 button.setBounds(x, y, 120, 60);  // 根據需求設置按鈕的大小
    //                 this.add(button);  // 將按鈕加入到面板中
        
    //                 System.out.println("Button for generator " + i + " initialized");
    //         }
    //         }
    //     }
    ...
    private void drawWindow(Graphics g) {
        for (ClientWindow clientWindow : clientGame.Window) {
            if(clientWindow != null) {
                ...
                g.drawImage(windowIcon, x, y, 60, 120, this);  
                System.out.println("Drawing window " + clientWindow.getId() + " at: (" + x + ", " + y + ")");
            }
        }
    }
